...
function updateTopic($db, $data) {
    ...
    if ($stmt->execute()) {
        if ($stmt->rowCount() > 0) {
            ...
            // No rows affected – data may be identical
            sendResponse([
                'success' => true,
...
try {
    ...
    switch ($resource) {
        case 'topics':
            switch ($method) {
                ...
                case 'DELETE':
                    ...
                    $topicId = isset($_GET['id'])
                        ? $_GET['id']
                        : (isset($data['topic_id']) ? $data['topic_id'] : null);
        ...
        case 'replies':
            switch ($method) {
                ...
                case 'DELETE':
                    $replyId = isset($_GET['id'])
                        ? $_GET['id']
                        : (isset($data['reply_id']) ? $data['reply_id'] : null);
        ...
        default:
            // Should be unreachable due to isValidResource, but just in case
            sendResponse([
                'success' => false,
    ...
    // Log internally if you have logging set up
