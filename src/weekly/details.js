...
// --- Element Selections ---
// TODO: Select the elements you added IDs for in step 2.
...
function renderWeekDetails(week) {
    if (!week) return;
    weekTitle.textContent = week.title || 'Week';
    ...
    if (Array.isArray(week.links) && week.links.length > 0) {
        ...
        weekLinksList.innerHTML = '<li>No resources available.</li>';
    }
}
...
function handleAddComment(event) {
    ...
    const text = (newCommentText && newCommentText.value) ? newCommentText.value.trim() : '';
    if (!text) return;

    const newComment = { author: 'Student', text: text };
...
async function initializePage() {
    ...
    try {
        ...
        if (!weeksRes.ok) throw new Error('Failed to load weeks.json');
        if (!commentsRes.ok) throw new Error('Failed to load comments.json');

        const weeksData = await weeksRes.json();
        const commentsData = await commentsRes.json();

        const week = Array.isArray(weeksData) ? weeksData.find(w => String(w.id) === String(currentWeekId)) : null;
        currentComments = commentsData && commentsData[currentWeekId] ? commentsData[currentWeekId].slice() : [];
